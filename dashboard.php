...
nav a{display:flex;align-items:center;gap:10px;padding:9px 16px;color:var(--muted);text-decoration:none;font-size:.75rem;letter-spacing:.5px;text-transform:uppercase;transition:.15s;border-left:2px solid transparent;}
...
.svc-card{background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:16px;position:relative;overflow:hidden;transition:.2s;}
...
.t-muted{color:var(--muted);} .t-red{color:var(--red);} .t-orange{color:var(--orange);} .t-cyan{color:var(--cyan);}
/* ATLAS */
...
/* Webmail */
.webmail-frame{width:100%;height:calc(100vh - 140px);border:none;border-radius:8px;background:var(--surface);}
/* Empty state */
.empty-state{text-align:center;padding:48px 24px;color:var(--muted);}
.empty-state .ico{font-size:2.5rem;margin-bottom:12px;}
.empty-state p{font-size:.78rem;line-height:1.8;}
  ...
  <div class="content">
    ...
    <div class="panel active" id="panel-overview">
      <div class="stats-row">
        ...
        <div class="stat-box"><div class="val" id="stat-alerts">—</div><div class="lbl">Alerts Today</div></div>
      </div>
    ...
    <div class="panel" id="panel-sentinel">
      <div class="stats-row" style="grid-template-columns:repeat(3,1fr);">
        <div class="stat-box"><div class="val" id="sent-ssh">—</div><div class="lbl">SSH Attempts</div></div>
...
<script>
const SERVICES = [
  {id:'lab-dashboard', name:'Lab Dashboard', port:8888, color:'cyan',   icon:'⚡', desc:'NOX control plane, WebSocket stats'},
  {id:'sentinel-agg',  name:'SENTINEL',      port:8282, color:'red',    icon:'🕯', desc:'Honeypot aggregator & alert pipeline'},
  {id:'sentinel-can',  name:'Canary Server', port:8181, color:'red',    icon:'🔔', desc:'HTTP canary token server'},
  {id:'villain-api',   name:'Villain-API',   port:8383, color:'purple', icon:'👾', desc:'C2 bridge — Villain framework'},
  {id:'nox-brain',     name:'NOX-BRAIN',     port:8484, color:'cyan',   icon:'🤖', desc:'AI analysis layer (Ollama)'},
  {id:'vault',         name:'VAULT',         port:8585, color:'orange', icon:'🔐', desc:'Credential intel — John/hashcat'},
  {id:'atlas',         name:'ATLAS',         port:8686, color:'purple', icon:'🎯', desc:'MITRE ATT&CK TTP tracker'},
  {id:'nox-command',   name:'NOX-COMMAND',   port:8787, color:'green',  icon:'⚙', desc:'Kill chain orchestrator'},
  {id:'honeyai',       name:'HoneyAI',       port:8191, color:'orange', icon:'🍯', desc:'AI SSH honeypot response gen'},
];
...
function renderAll(data) {
  ...
  document.getElementById('stat-alerts').textContent   = data.sentinel?.alerts_today ?? '—';
}
...
function renderServices(svcs) {
  document.getElementById('serviceGrid').innerHTML = SERVICES.map(s => {
    ...
    return `<div class="svc-card ${s.color}">
...
function renderMachines(machines) {
  ...
  document.getElementById('machineGrid').innerHTML = machines.map(m => {
    ...
    return `<div class="machine-card">
      ...
      <div class="mstats">
        <div class="mstat"><div class="v">${cpu}%</div><div class="l">CPU</div><div class="progress"><div class="progress-bar cpu" style="width:${cpu}%"></div></div></div>
        <div class="mstat"><div class="v">${mem}%</div><div class="l">MEM</div><div class="progress"><div class="progress-bar mem" style="width:${mem}%"></div></div></div>
...
function renderSentinel(s) {
  ...
  if (s.recent_events?.length)
    document.getElementById('sentinelLog').innerHTML = s.recent_events.map(e=>
      `<div><span class="t-muted">[${e.ts||''}]</span> <span class="${e.severity==='high'?'t-red':'t-orange'}">${e.src_ip||''}</span> ${e.message||''}</div>`
    ).join('');
}
...
function renderHoneyAI(h) {
  ...
  if (h.sessions?.length)
    document.getElementById('honeyaiLog').innerHTML = h.sessions.map(s=>
      `<div><span class="t-cyan">[${s.ip||'?'}]</span> <span class="t-muted">TTPs: ${(s.ttps||[]).join(', ')||'none'}</span> cmds:${s.command_count||0}</div>`
    ).join('');
}
...
async function fetchEvents() {
  try {
    ...
    document.getElementById('eventFeed').innerHTML = d.events.map(e=>{
      ...
      const tc={sentinel:'sentinel',atlas:'atlas',system:'system',honeyai:'honeyai'}[e.type]||'system';
